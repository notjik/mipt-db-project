...
BEGIN
    IF TG_TABLE_NAME = 'users' THEN
        INSERT INTO mipt_project.users_history (user_id, username, email, password_hash, created_at, operation_type)
        VALUES (CASE TG_OP WHEN 'DELETE' THEN OLD.id ELSE NEW.id END,
    ...
    ELSIF TG_TABLE_NAME = 'artists' THEN
        INSERT INTO mipt_project.artists_history (artist_id, name, bio, user_id, created_at, operation_type)
        VALUES (CASE TG_OP WHEN 'DELETE' THEN OLD.id ELSE NEW.id END,
    ...
    ELSIF TG_TABLE_NAME = 'genres' THEN
        INSERT INTO mipt_project.genres_history (genre_id, name, description, operation_type)
        VALUES (CASE TG_OP WHEN 'DELETE' THEN OLD.id ELSE NEW.id END,
                CASE TG_OP WHEN 'DELETE' THEN OLD.name ELSE NEW.name END,
                CASE TG_OP WHEN 'DELETE' THEN OLD.description ELSE NEW.description END,
                TG_OP);
    ELSIF TG_TABLE_NAME = 'albums' THEN
        INSERT INTO mipt_project.albums_history (album_id, artist_id, title, release_date, created_at, operation_type)
        VALUES (CASE TG_OP WHEN 'DELETE' THEN OLD.id ELSE NEW.id END,
    ...
    ELSIF TG_TABLE_NAME = 'tracks' THEN
        INSERT INTO mipt_project.tracks_history (track_id, album_id, title, duration, track_number, track_url,
    ...
    ELSIF TG_TABLE_NAME = 'playlists' THEN
        INSERT INTO mipt_project.playlists_history (playlist_id, user_id, title, description, created_at,
                                                    operation_type)
        VALUES (CASE TG_OP WHEN 'DELETE' THEN OLD.id ELSE NEW.id END,
                CASE TG_OP WHEN 'DELETE' THEN OLD.user_id ELSE NEW.user_id END,
                CASE TG_OP WHEN 'DELETE' THEN OLD.title ELSE NEW.title END,
...
BEGIN
    IF TG_TABLE_NAME = 'track_genres' THEN
        INSERT INTO mipt_project.track_genres_history (track_id, genre_id, operation_type)
        VALUES (CASE TG_OP WHEN 'DELETE' THEN OLD.track_id ELSE NEW.track_id END,
                CASE TG_OP WHEN 'DELETE' THEN OLD.genre_id ELSE NEW.genre_id END,
                TG_OP);
    ELSIF TG_TABLE_NAME = 'playlist_tracks' THEN
        INSERT INTO mipt_project.playlist_tracks_history (playlist_id, track_id, position, operation_type)
        VALUES (CASE TG_OP WHEN 'DELETE' THEN OLD.playlist_id ELSE NEW.playlist_id END,
...
-- Триггер для автоматического создания плейлиста "Любимые треки" при добавлении нового пользователя
CREATE OR REPLACE FUNCTION mipt_project.create_default_favorites_playlist() RETURNS TRIGGER AS
$$
BEGIN
    INSERT INTO mipt_project.playlists (user_id,
                                        title,
                                        description)
    VALUES (NEW.id,
            'Любимые треки',
...
CREATE TRIGGER users_audit
    ...
    ON mipt_project.users
    FOR EACH ROW
EXECUTE FUNCTION mipt_project.log_entity_history();

-- artists
DROP TRIGGER IF EXISTS artists_audit ON mipt_project.artists;
CREATE TRIGGER artists_audit
    AFTER INSERT OR UPDATE OR DELETE
    ON mipt_project.artists
    FOR EACH ROW
EXECUTE FUNCTION mipt_project.log_entity_history();
